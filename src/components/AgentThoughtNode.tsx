...
const TOOL: Record<string, { label: string; icon: React.ReactNode; tint: string }> = {
  ...
  checkPolymarketMarkets:{ label: "Scanning Polymarket",      icon: <IconDice />,   tint: "#F06292" },
  assessProbability:     { label: "Computing edge",           icon: <IconChart />,  tint: "#F06292" },
  placePolymarketBet:    { label: "Placing bet",              icon: <IconDice />,   tint: "#F06292" },
...
export function AgentThoughtNode({ data, selected }: { data: ThoughtNodeData; selected?: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "11px 14px 12px 16px" }}>
...
function MediaNode({ content }: { content: Record<string, unknown> }) {
  ...
  return (
    <>
      ...
      <div style={{ fontSize: 10, color: MID, marginTop: 6, letterSpacing: "0.04em" }}>via Venice · x402</div>
    </>
  );
}
...
function IconDice()    { return svg(<><rect x="4" y="4" width="16" height="16" rx="3" /><circle cx="9" cy="9" r="1" fill="currentColor" /><circle cx="15" cy="15" r="1" fill="currentColor" /><circle cx="15" cy="9" r="1" fill="currentColor" /><circle cx="9" cy="15" r="1" fill="currentColor" /></>); }
